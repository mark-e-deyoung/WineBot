...
4. **Release v0.9.3:**
   ...
   - **Note:** Automated CI release workflow (`release.yml`) persistently fails on the `smoke-test` phase due to a "Wait for Windows" timeout on GitHub hosted runners, despite passing locally.
   - **Mitigation:** Production images (`v0.9.3-rel` and `v0.9.3-rel-runner`) were built locally and pushed to GHCR manually.
