...
async def test_policy_default_mode(auth_headers):
    ...
    response = client.post("/input/mouse/click", json={"x": 0, "y": 0}, headers=auth_headers)
...
async def test_policy_grant_control(auth_headers):
    ...
    # User grants control
    response = client.post("/sessions/test-session/control/grant", json={"lease_seconds": 10}, headers=auth_headers)
...
async def test_policy_user_override(auth_headers):
    ...
    # Simulate user input
    response = client.post("/input/client/event", json={"event": "mousemove"}, headers=auth_headers)
    assert response.status_code == 200
...
async def test_policy_stop_now(auth_headers):
    ...
    response = client.post("/sessions/test-session/user_intent", json={"intent": "STOP_NOW"}, headers=auth_headers)
    assert response.status_code == 200
